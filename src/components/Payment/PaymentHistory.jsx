...
/**
 * PaymentHistory — US5: Student Payments
 * Shows a student's full payment history fetched from Firestore.
 */
export default function PaymentHistory({ currentUser }) {
  ...
  async function loadPayments() {
    try {
      const data = await paymentService.getUserPayments(currentUser.uid);
      setPayments(data);
    } catch (err) {
  ...
  function formatDate(ts) {
    ...
    return d.toLocaleString('en-ZA', { day: '2-digit', month: 'short', year: 'numeric',
                ...
                <div key={p.id} className="history-card">
                  ...
                  {p.items && p.items.length > 0 && (
                    <div className="hcard-items">
                      {p.items.map((item, i) => (
                        <span key={i} className="hcard-item-tag">
                          {item.name} ×{item.qty}
                        </span>
                      ))}
                    </div>
